...
export class AuthService {
  ...
  login(credentials: { email: string, senha: string }): Observable<Usuario> {
    ...
    const anotherStudent: Usuario = { id: 104, nome_completo: 'Aluna Ana Clara', email: 'ana.clara@utfpr.edu.br', papel: { id: 3, nome: 'Aluno', pode_ser_responsavel: false }};

    if (credentials.email === adminUser.email && credentials.senha === 'admin123') {
      this.currentUser = adminUser;
      return of(this.currentUser).pipe(delay(300));
    } else if (credentials.email === profUser.email && credentials.senha === 'prof123') {
      this.currentUser = profUser;
      return of(this.currentUser).pipe(delay(300));
    } else if (credentials.email === studentUser.email && credentials.senha === '123') {
      this.currentUser = studentUser;
      return of(this.currentUser).pipe(delay(300));
    } else if (credentials.email === anotherStudent.email && credentials.senha === '456') {
      this.currentUser = anotherStudent;
      return of(this.currentUser).pipe(delay(300));
    } else {
